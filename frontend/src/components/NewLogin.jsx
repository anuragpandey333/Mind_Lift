    ...
    <div className={`min-h-screen flex items-center justify-center p-4 transition-all duration-700 ${
      ...
      <div className={`max-w-md w-full rounded-2xl shadow-2xl p-8 border transition-all duration-500 ${
        ...
        <div className="flex items-center my-5">
          ...
          <span className={`mx-3 text-sm ${isToggled ? 'text-[#94a3b8]' : 'text-gray-400'}`}>or</span>
          <div className={`flex-1 h-px ${isToggled ? 'bg-[#BCCCDC]/30' : 'bg-[#D9EAFD]'}`} />
        </div>

        <button
          onClick={() => window.location.href = `${import.meta.env.VITE_API_URL}/api/auth/google`}
          className={`w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border font-semibold transition-all duration-300 hover:scale-105 ${
            isToggled
              ? 'border-[#BCCCDC]/30 bg-[#000000]/40 text-[#D9EAFD] hover:bg-[#BCCCDC]/10'
              : 'border-[#D9EAFD] bg-white text-[#1a202c] hover:bg-[#D9EAFD]/40'
          }`}
        >
